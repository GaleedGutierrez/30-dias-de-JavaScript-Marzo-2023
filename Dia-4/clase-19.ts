import { InterfaceStudentAverage, InterfaceStudentOriginal } from './interface';

function getStudentAverage (students: InterfaceStudentOriginal[]) {
	const AVERAGE_FOR_EACH_STUDENTS: InterfaceStudentAverage[] = students.map((student) => {
		const { name, grades } = student;
		const SUM = grades.reduce((acumulador, actual) => acumulador + actual, 0);
	...
	const SUM_GRADES = GRADES.reduce((acumulador, actual) => acumulador + actual);
