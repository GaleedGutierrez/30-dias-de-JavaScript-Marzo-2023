import { InterfaceFamousKitty } from './interface';

export function findFamousCats(cats: InterfaceFamousKitty[]) {
	...
	const THE_MOST_FAMOUS = SUM_KITTY_FOLLOWERS.filter(
		(cat) => cat.followers === MAX_FOLLOWERS
	).map((cat) => cat.name);
